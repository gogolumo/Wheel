...
final class WheelPinnedSlotStoreTests: XCTestCase {
    func testPinPersistsAndRestores() {
        ...
        XCTAssertEqual(WheelPinnedSlotStore(defaults: defaults).application(at: 0), app)
    }

    func testReducingDirectionCountDoesNotDeleteHiddenPins() {
        let suite = "WheelPinnedSlotStoreTests.\(UUID().uuidString)"
        let defaults = UserDefaults(suiteName: suite)!
        defer { defaults.removePersistentDomain(forName: suite) }
    ...
    func testPinnedApplicationIsNotDuplicatedInDynamicSlots() {
        let store = WheelPinnedSlotStore(defaults: UserDefaults(suiteName: UUID().uuidString)!)
        let now = Date()
        let whatsapp = WheelApplicationContext(
            stableIdentifier: "bundle:whatsapp", localizedName: "WhatsApp",
            bundleIdentifier: "whatsapp", applicationURL: URL(fileURLWithPath: "/Applications/WhatsApp.app"),
            firstSeenAt: now, lastActivatedAt: now, runState: .running
        )
        let safari = WheelApplicationContext(
            stableIdentifier: "bundle:safari", localizedName: "Safari",
            bundleIdentifier: "safari", applicationURL: URL(fileURLWithPath: "/Applications/Safari.app"),
            firstSeenAt: now, lastActivatedAt: now, runState: .running
        )
        store.pin(.init(context: whatsapp), at: 0)
        let merged = store.merge(dynamic: [whatsapp, safari], directionCount: 4).compactMap { $0 }
        XCTAssertEqual(merged.filter { $0.stableIdentifier == whatsapp.stableIdentifier }.count, 1)
        XCTAssertTrue(merged.contains { $0.stableIdentifier == safari.stableIdentifier })
    }
